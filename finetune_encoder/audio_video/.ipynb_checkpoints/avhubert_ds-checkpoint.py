...
#         print(padded_audio)
    ...
    def fn4mp(self, idx):
        a,v = self.__getsingle__(idx)
        return self.collate([a], [v])

    def __cache__(self):
        self.cached = True
        self.video_feats_list = []
        self.audio_feats_list = []
        self.fn_list = list(self.df_["smp_id"])
        for idx in tqdm(range(len(self.fn_list))):
            res = self.__getsingle__(idx)     
            self.video_feats_list.append(res[1])
            self.audio_feats_list.append(res[0])   
