...
def handle_values(fileNAME, PATH, writer, rate):
    ...
    with open(PATH, 'r', encoding='latin-1') as fid:
        ...
        # Save values to DataFrame
        save_dataFrame(iframe, fframe, emotion, emovector,
                       speaker, smp_id, fileNAME, writer)
...
### Function to create dataFrame using pandas ###
def save_dataFrame(iframe, fframe, emotion, emovector, speaker, smp_id, fileNAME, writer):

    # Append all data to columns as shown below
    data = {
        'iframe': iframe, 'fframe': fframe, 'emotion': emotion, 'speaker': speaker, "smp_id": smp_id,
        'emovectorA': [emovector[i][0] for i in range(np.shape(emovector)[0])],
    ...
    writer._save()
    # sorted_df.to_csv(os.path.join('E:/datasets/preprocessed/extractionmap', fileNAME+'.csv'))
    # Function DOESN'T return anything
    pass
...
def main():
    iemocapPATH = "E:/datasets/IEMOCAP_full_release.tar/IEMOCAP_full_release/IEMOCAP_full_release"

    # Folder to store the extractionmaps
    create_folder('E:/datasets/preprocessed')
    ...
    for ses in range(1, n_sessions+1):
        ...
        EMO_EVAL_PATH = '{}/Session'.format(iemocapPATH) + \
            str(ses)+'/dialog/EmoEvaluation/'
        # Take list of files in EMO_EVAL_PATH
        evaluations = os.listdir(EMO_EVAL_PATH)
        # Skip non .txt files-folders
        # exclude apple file
        evaluations = [i for i in evaluations if i[-4:]
    ...
    # Execution time
    print(
        'Execution time of extractionmapCreator.py [sec]: ' + str(time.time() - t0))
