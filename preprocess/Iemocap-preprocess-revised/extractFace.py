...
def crop_image(image, left_precenter, right_precenter, sheet, speaker):
    ...
    cv2.rectangle(img1, (left_a, left_b),
                  (left_a+width, left_b+height), (0, 0, 255), 2)
    ...
    cv2.rectangle(img2, (right_a, right_b),
                  (right_a+width, right_b+height), (0, 0, 255), 2)
...
def main():
    ...
    imagePATH = 'E:/datasets/preprocessed/face'
    ...
    for ses in range(1, 6):
        ...
        sessionPATH = 'E:/datasets/IEMOCAP_full_release.tar/IEMOCAP_full_release/IEMOCAP_full_release/Session' + \
            str(ses)+'/dialog/avi/DivX'
        videos = [video for video in os.listdir(sessionPATH) if not video.startswith("._") and video.endswith(".avi")]
        ...
        for vid in range(len(videos)):
            if str(videos[vid]).startswith("._"):
                continue
            # Current video path
            ...
            if os.path.isfile(videoPATH):
                ...
                while success:
                    ...
                    if True: # currentFrame > 9550 and currentFrame < 10074:

                        # Find current speaker
                        currentSpeaker, currentRow = find_speaker(
                            currentFrame, currentRow, xl, sheet)
                        # Crop frame
                        frame, left_precenter, right_precenter = crop_image(
                            frame, left_precenter, right_precenter, sheet, currentSpeaker)
                        # Check that current frame refers to speaker
                        if np.shape(frame)[0] > 2:
                            # Save current cropped image in .jpg file
                            name = str(image_videoPATH) + '/frame' + \
        ...
        # Execution time
        print(
            'Execution time of extractFace.py [sec]: ' + str(time.time() - t0))
